...
public class ItemStackCodecMixin {
	...
	private ItemStack setModifiedStackSize(ItemStack stack) {
		if (ShulkerFixSettings.clientShulkerSync &&
			stack.getOrDefault(MAX_STACK_SIZE, 1) < stack.getCount() &&
			!stack.getOrDefault(CUSTOM_DATA, NbtComponent.DEFAULT).contains(MARKER) &&
			!Util.hasCustomMaxStackSize(stack)
		) {
			stack = stack.copy();
			stack.set(MAX_STACK_SIZE, stack.getMaxCount());
			NbtComponent.set(CUSTOM_DATA, stack, nbt -> nbt.putBoolean(MARKER, true));
		}
		return stack;
	}
	...
	private ItemStack getModifiedStackSize(ItemStack stack) {
		...
		if (stack.getOrDefault(CUSTOM_DATA, NbtComponent.DEFAULT).contains(MARKER)) {
			stack = stack.copy();
			stack.set(MAX_STACK_SIZE, stack.getDefaultComponents().get(MAX_STACK_SIZE));
			stack.capCount(stack.getMaxCount());
			NbtComponent.set(CUSTOM_DATA, stack, nbt -> nbt.remove(MARKER));
		}
		return stack;
	}
}
